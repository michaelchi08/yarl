...
#include "prototype/kinematics/quadrotor.hpp"
...
TEST(AttitudeController, calculate) {
  ...
  for (int i = 0; i < 1000; i++) {
    actual << quad.states(0), quad.states(1), quad.states(2), quad.states(8);
...
TEST(PositionController, calculate) {
  ...
  for (int i = 0; i < 5000; i++) {
    // position controller
    // clang-format off
    actual_position << quad.states(6), quad.states(7), quad.states(8), quad.states(2);
