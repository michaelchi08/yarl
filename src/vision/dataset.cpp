    ...
    if (f_2d(2) >= 1.0) {
      ...
      if ((f_2d(0) < this->image_width) && (f_2d(0) > 0)) {
        if ((f_2d(1) < this->image_height) && (f_2d(1) > 0)) {
          observed.push_back({f_2d.block(0, 0, 2, 1), landmark.second});
        }
      }
    }
  }
...
int VOTestDataset::configure(const std::string &config_file) {
  ...
  parser.addParam("/config/landmarks/nb_landmarks", &this->nb_landmarks);
  parser.addParam("/config/landmarks/x/min", &this->landmark_x_bounds(0));
  parser.addParam("/config/landmarks/x/max", &this->landmark_x_bounds(1));
  parser.addParam("/config/landmarks/y/min", &this->landmark_y_bounds(0));
  parser.addParam("/config/landmarks/y/max", &this->landmark_y_bounds(1));
  parser.addParam("/config/landmarks/z/min", &this->landmark_z_bounds(0));
  parser.addParam("/config/landmarks/z/max", &this->landmark_z_bounds(1));
...
int VOTestDataset::generateLandmarks() {
  ...
  for (int i = 0; i < this->nb_landmarks; i++) {
    double x = randf(this->landmark_x_bounds(0), this->landmark_x_bounds(1));
    double y = randf(this->landmark_y_bounds(0), this->landmark_y_bounds(1));
    double z = randf(this->landmark_z_bounds(0), this->landmark_z_bounds(1));
