...
int ORB::configure(void) {
  ...
  this->detector =
      cv::ORB(this->nb_features, this->scale_factor, this->nb_levels,
              this->edge_threshold, this->first_level, this->wta_k,
              this->score_type, this->patch_size);
...
int ORB::compute(cv::Mat &image, std::vector<cv::KeyPoint> &keypoints,
                 cv::Mat &descriptors) {
